...
if 'flowers_sprinkled' not in st.session_state:
    ...
    for(let i=0; i<80; i++) {{
        ...
        f.style.left = Math.random() * 100 + 'vw'; f.style.fontSize = (Math.random() * 20 + 15) + 'px';
        f.style.opacity = Math.random() * 0.6 + 0.4;
        let duration = Math.random() * 4 + 2; f.style.animation = `fallDown ${{duration}}s linear forwards`;
        document.body.appendChild(f); setTimeout(() => f.remove(), duration * 1000);
    }}
